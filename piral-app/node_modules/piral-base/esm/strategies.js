...
export function syncStrategy(options, cb) {
    ...
    return registerDependencies(dependencies).then(() => runAll(createApi, pilets, [], hooks).then((integratedPilets) => cb(undefined, integratedPilets), (err) => cb(err, [])));
}
/**
 ...
 */
export function createDeferredStrategy(trigger, strategy = standardStrategy) {
    return (options, cb) => {
        cb(undefined, []);
