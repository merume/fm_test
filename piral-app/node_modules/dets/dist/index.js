...
function includeTypings(context, node) {
    if ((0, helpers_1.shouldInclude)(node)) {
...
function stringifyModule(name, refs) {
    const content = (0, exports_1.stringifyExports)(refs);
    const formattedContent = (0, helpers_1.formatContent)(content);
...
class DeclVisitor {
    ...
    normalizeName(node) {
        ...
        const global = (0, helpers_1.isGlobal)(symbol);
        const { moduleName, lib, symbolName } = (0, utils_1.getPackage)(node, global, c.root, c.availableImports);
        if (!lib) {
            const name = global ? (0, helpers_1.fullyQualifiedName)(symbol, '_') : (0, helpers_1.getSymbolName)(symbol);
            return this.getName(node, name);
        }
        else if (global) {
            return (0, helpers_1.fullyQualifiedName)(symbol, '.');
        }
        else {
            return (0, utils_1.createBinding)(c, moduleName, symbolName !== null && symbolName !== void 0 ? symbolName : (0, helpers_1.getSymbolName)(symbol));
        }
    }
    ...
    getNode(node) {
        if (ts.isTypeNode(node)) {
            ...
            this.enqueue(node);
            return (0, utils_1.getRef)(this.normalizeName(node), this.getTypeParameters(node.typeParameters));
        }
        else if ((0, helpers_1.isDefaultExport)(node) || ts.isVariableDeclaration(node) || ts.isVariableStatement(node)) {
            this.enqueue(node);
            return (0, utils_1.getRef)(this.normalizeName(node));
        }
        else if (ts.isPropertyAssignment(node)) {
            return {
                kind: 'prop',
                modifiers: (0, helpers_1.getModifiers)(node.symbol),
        ...
        return (0, utils_1.getRef)(node.symbol.name);
    }
    ...
    getNormalProp(node) {
        ...
        const comment = (0, helpers_1.getCommentOrDrop)(checker, (_b = (_a = node.emitNode) === null || _a === void 0 ? void 0 : _a.commentRange) !== null && _b !== void 0 ? _b : node, canDrop);
    ...
    getConstructor(node) {
        return {
            kind: 'constructor',
            parameters: this.getFunctionParameters(node.parameters),
            comment: (0, helpers_1.getComment)(this.context.checker, node),
        };
    }
    getClassMember(node) {
        const { checker, flags } = this.context;
        const canDrop = !flags.noIgnore;
        const comment = (0, helpers_1.getCommentOrDrop)(checker, node, canDrop);
        if (typeof comment === 'string') {
            return {
                kind: 'prop',
    ...
    getTypeParameter(node) {
        return {
            kind: 'typeParameter',
            parameter: (0, utils_1.getRef)(node.name.text),
    ...
    getFunctionParameter(node) {
        return {
            kind: 'parameter',
            param: (0, helpers_1.getParameterName)(node.name),
    ...
    getPredicate(node) {
        return {
            kind: 'predicate',
            name: (0, helpers_1.getPredicateName)(node.parameterName),
            value: this.getTypeNode(node.type),
        };
    }
    getSetAccessor(node) {
    ...
    getTypeReference(node) {
        const c = this.context.checker;
        const decl = (0, helpers_1.getDeclarationFromNode)(c, node);
        if (decl && !ts.isTypeParameterDeclaration(decl)) {
            this.enqueue(decl);
            return (0, utils_1.getRef)(this.normalizeName(decl), this.getTypeArguments(node.typeArguments));
        }
        return (0, utils_1.getRef)((0, helpers_1.getTypeRefName)(node.typeName), this.getTypeArguments(node.typeArguments));
    }
    getTypeLiteral(node) {
        return {
            kind: 'interface',
    ...
    getExpressionWithTypeArguments(node) {
        const decl = (0, helpers_1.getDeclarationFromNode)(this.context.checker, node.expression);
        this.enqueue(decl);
        return (0, utils_1.getRef)(this.normalizeName(decl), this.getTypeArguments(node.typeArguments));
    }
    getArray(node) {
        return (0, utils_1.getRef)('Array', [this.getTypeNode(node.elementType)]);
    }
    getInfer(node) {
        return {
            kind: 'infer',
    ...
    getTypeQueryNode(node) {
        ...
        const decl = (0, helpers_1.getDeclarationFromNode)(this.context.checker, node.exprName);
        this.enqueue(decl);
        return (0, utils_1.getRef)(`typeof ${(0, helpers_1.getTypeRefName)(node.exprName)}`);
    }
    ...
    getDefaultExpression(node) {
        var _a;
        const name = (_a = (0, helpers_1.getExportName)(node.name)) !== null && _a !== void 0 ? _a : '_default';
        const expr = node.expression;
        if (ts.isIdentifier(expr)) {
            const decl = (0, helpers_1.getDeclarationFromNode)(this.context.checker, expr);
            this.enqueue(decl);
            return (0, utils_1.getRef)(expr.text);
        }
    ...
    getClass(node) {
        ...
        const docs = (0, helpers_1.getAllJsDocs)(checker, decls);
        ...
        return {
            ...
            types: this.getTypeParameters(node.typeParameters),
            comment: (0, helpers_1.stringifyJsDocs)(docs),
        };
    }
    getPropName(name) {
        if (!name) {
    ...
    getInterface(node) {
        ...
        const decls = type.symbol.declarations
            ...
            .filter((m) => !(0, utils_1.isImportedFile)(m, root, availableImports));
        const clauses = [];
        const props = [];
        ...
        decls.forEach((m) => {
            ...
            (_a = m.heritageClauses) === null || _a === void 0 ? void 0 : _a.forEach((c) => {
                clauses.includes(c) || (0, includes_1.includeClauses)(this.context, clauses, c, docs.tags);
            });
            (_b = m.members) === null || _b === void 0 ? void 0 : _b.forEach((p) => {
                props.includes(p) || (0, includes_1.includeProp)(props, p, prop => this.getPropName(prop), docs.tags);
            });
    ...
    getExpression(node) {
        if (ts.isArrowFunction(node)) {
            ...
            const decl = (0, helpers_1.getDeclarationFromNode)(this.context.checker, node);
            this.enqueue(decl);
            return (0, utils_1.getRef)(node.text);
        }
        else {
    ...
    getEnum(node) {
        const symbol = (0, helpers_1.getSymbol)(this.context.checker, node);
        const name = this.getName(node, node.name.text);
        return {
            kind: 'enumLiteral',
            name,
    ...
    includeInContext(node, createType) {
        const c = this.context;
        const symbol = (0, helpers_1.getSymbol)(c.checker, node);
        const global = (0, helpers_1.isGlobal)(symbol);
        const { external, fn } = (0, utils_1.getPackage)(node, global, c.root, c.availableImports);
        if (!external) {
    ...
    includeDefaultExport(node) {
        const expr = node.expression;
        if (expr) {
            this.includeInContext(expr, () => (0, utils_1.getDefault)(this.getDefaultExpression(node)));
        }
        else if (ts.isFunctionDeclaration(node)) {
            const name = '_default';
            this.includeInContext(node, () => (Object.assign(Object.assign({}, this.getMethodSignature(node)), { name })));
            this.includeInContext(node, () => (0, utils_1.getDefault)((0, utils_1.getRef)(name)));
        }
        else if (ts.isClassDeclaration(node)) {
            this.includeInContext(node, () => (0, utils_1.getDefault)(this.getClass(node)));
        }
        else {
    ...
    includeSelectedExports(elements) {
        ...
        elements.forEach((el) => {
            if (el.symbol) {
                ...
                if (original) {
                    const decl = (0, helpers_1.getDeclarationFromSymbol)(this.context.checker, original);
                    if (decl) {
                    ...
                    if (symbol) {
                        const decl = (0, helpers_1.getDeclarationFromSymbol)(this.context.checker, symbol);
                        if (decl) {
                            this.processNode(decl);
    ...
    processModule(node) {
        ...
        (_a = node.body) === null || _a === void 0 ? void 0 : _a.forEachChild((subNode) => {
            if ((0, helpers_1.isNodeExported)(subNode) || availableImportNames.includes(name)) {
    ...
    processNode(node) {
        ...
        else if ((0, helpers_1.isDefaultExport)(node)) {
...
function includeApi(context, node, interfaceName) {
    if ((0, typescript_1.isInterfaceDeclaration)(node) && node.name.text === interfaceName) {
        context.exports.push(node);
    }
    else {
...
function createBinding(context, lib, name) {
    ...
    return `${(0, helpers_1.getLibRefName)(lib)}.${name}`;
}
...
function isImportedFile(node, root, imports) {
    ...
    if (fn) {
        const libName = (0, helpers_1.getLibName)(fn, root);
...
function getPackage(node, global, root, imports) {
    var _a;
    const fn = (_a = node.getSourceFile()) === null || _a === void 0 ? void 0 : _a.fileName;
    const base = (0, helpers_1.isBaseLib)(fn) || false;
    if (!base) {
        const libName = (0, helpers_1.getLibName)(fn, root);
        ...
        return {
            external: !!lib,
            moduleName: (lib && (0, helpers_1.getModule)(node)) || lib,
            symbolName,
            base,
            lib,
            fn,
...
function fillExportsFromApi(context, apiPath, apiName) {
    ...
    if (api) {
        ts.forEachChild(api, (node) => (0, input_1.includeApi)(context, node, apiName));
    }
    else {
...
function fillExportsFromTypes(context, typingsPath) {
    ...
    if (tp) {
        ts.forEachChild(tp, (node) => (0, input_1.includeTypings)(context, node));
    }
    else {
    ...
    for (const sourceFile of sourceFiles) {
        ...
        (_a = sourceFile.resolvedModules) === null || _a === void 0 ? void 0 : _a.forEach((value, key) => {
            ...
            if (!fileName) {
                ...
                (0, input_1.includeExports)(context, key, file === null || file === void 0 ? void 0 : file.symbol);
...
function addAmbientModules(context, imports) {
    ...
    for (const module of modules) {
        ...
        const lib = (0, helpers_1.getLibName)(file, context.root);
        if (imports.includes(lib)) {
            (0, input_1.includeExports)(context, module.name, module);
        }
    }
}
...
function generateDeclaration(options) {
    const { name, root = process.cwd(), imports = [], files = [], types = [], apis = [], plugins = [], logger = logger_1.defaultLogger, logLevel = 3, noIgnore = false, noModuleDeclaration = false, } = options;
    const log = (0, logger_1.wrapLogger)(logger, logLevel);
    log.verbose(`Aggregating the sources from "${root}".`);
    const sources = [
        ...files.map((file) => (0, helpers_1.findAppRoot)(root, file)),
        ...apis.map((api) => (0, helpers_1.findAppRoot)(root, api.file)),
        ...types.map((type) => (0, helpers_1.findAppRoot)(root, type)),
    ];
    ...
    for (const api of apis) {
        const path = (0, helpers_1.findAppRoot)(root, api.file);
        fillExportsFromApi(context, path, api.name);
    }
    log.verbose(`Starting type aggregation from "${root}".`);
    for (const type of types) {
        const path = (0, helpers_1.findAppRoot)(root, type);
        fillExportsFromTypes(context, path);
    }
    log.verbose(`Processing the visitor context.`);
    processVisitorContext(context, plugins);
    return (0, output_1.stringifyDeclaration)(context);
}
exports.generateDeclaration = generateDeclaration;
function retrieveTypings(options) {
    const name = 'main';
    const { root = process.cwd(), imports = [], files = [], types = [], plugins = [], logger = logger_1.defaultLogger, logLevel = 3, noIgnore = false, noModuleDeclaration = false, } = options;
    const log = (0, logger_1.wrapLogger)(logger, logLevel);
    log.verbose(`Aggregating the sources from "${root}".`);
    const sources = [...files.map((file) => (0, helpers_1.findAppRoot)(root, file)), ...types.map((type) => (0, helpers_1.findAppRoot)(root, type))];
    log.verbose(`Setting up a visitor context for "${name}".`);
    const context = setupVisitorContext(name, root, sources, imports, log, {
        noIgnore,
        noModuleDeclaration,
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.getGlobalName = exports.getKeyName = exports.getLibName = exports.getParameterName = exports.getParameterElement = exports.getExportName = exports.getPredicateName = exports.getTypeRefName = exports.getLibRefName = exports.isAnonymous = void 0;
const typescript_1 = __webpack_require__(34);
...
function getExportName(name) {
    ...
    else if ((0, typescript_1.isIdentifier)(name)) {
        return name.text;
    }
    else if ((0, typescript_1.isStringLiteral)(name)) {
        return name.text;
    } /* is NumericLiteral */
    else {
        return name.text;
...
function getParameterElement(element) {
    return (0, typescript_1.isBindingElement)(element) ? getParameterName(element.name) : getParameterName(element);
}
...
function getParameterName(name) {
    ...
    else if ((0, typescript_1.isObjectBindingPattern)(name)) {
        const content = name.elements.map(getParameterElement).join(', ');
        return `{ ${content} }`;
    }
    else if ((0, typescript_1.isArrayBindingPattern)(name)) {
        const content = name.elements.map(getParameterElement).join(', ');
        return `[${content}]`;
...
function makeModule(fileName, root) {
    const relFile = (0, path_1.relative)(root, fileName);
...
function getModule(node) {
    while (node) {
        ...
        if ((0, typescript_1.isModuleDeclaration)(node) && (0, typescript_1.isSourceFile)(node.parent) && (0, typescript_1.isStringLiteral)(node.name)) {
...
function getDeclarationFromSymbol(checker, symbol) {
    ...
    if (!symbol) {
        ...
        if (decl && (0, typescript_1.isImportSpecifier)(decl)) {
...
function getSymbol(checker, node) {
    ...
    else if ((0, typescript_1.isTypeReferenceNode)(node)) {
...
function shouldInclude(node) {
    return (0, typescript_1.isModuleDeclaration)(node) || (0, typescript_1.isExportDeclaration)(node) || isNodeExported(node);
}
exports.shouldInclude = shouldInclude;
function isNodeExported(node, alsoTopLevel = false) {
    return ((0, typescript_1.isExportAssignment)(node) ||
        ((0, typescript_1.getCombinedModifierFlags)(node) & typescript_1.ModifierFlags.Export) !== 0 ||
...
function isKeyOfType(type) {
    return type && (0, typescript_1.isTypeOperatorNode)(type) && type.operator === typescript_1.SyntaxKind.KeyOfKeyword;
}
exports.isKeyOfType = isKeyOfType;
function isUnionType(type) {
    return type && (0, typescript_1.isUnionTypeNode)(type);
}
exports.isUnionType = isUnionType;
function isIdentifierType(type) {
    return type && (0, typescript_1.isTypeReferenceNode)(type) && (0, typescript_1.isIdentifier)(type.typeName);
}
exports.isIdentifierType = isIdentifierType;
function isInferType(type) {
    return type && (0, typescript_1.isInferTypeNode)(type);
}
...
function findAppRoot(root, app) {
    return (0, path_1.resolve)(root, app);
}
...
function getJsDocs(checker, node) {
    var _a, _b;
    if ((0, typescript_1.isMethodDeclaration)(node) || (0, typescript_1.isMethodSignature)(node) || (0, typescript_1.isCallSignatureDeclaration)(node)) {
...
function stringifyJsDocs(doc) {
    ...
    const result = (_b = (_a = doc.comment) === null || _a === void 0 ? void 0 : _a.map((m) => m.text)) !== null && _b !== void 0 ? _b : [];
    ...
    if (symbol) {
        context.checker.getExportsOfModule(symbol).forEach((exp) => {
            ...
            if (!decl) {
                ...
                if (name) {
                    defs[name] = (0, helpers_1.getDeclarationFromNode)(context.checker, decl);
                }
            }
            else if (ts.isExportAssignment(decl)) {
                defs['default'] = (0, helpers_1.getDeclarationFromNode)(context.checker, decl);
            }
            else if (ts.isVariableDeclaration(decl)) {
                defs[(0, helpers_1.getParameterName)(decl.name)] = decl;
            }
                ...
                const name = (0, helpers_1.isDefaultExport)(decl) ? 'default' : (_c = decl.name) === null || _c === void 0 ? void 0 : _c.text;
                if (name) {
                    defs[name] = decl;
                }
            }
